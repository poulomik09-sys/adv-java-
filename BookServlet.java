...
Connection con = DriverManager.getConnection(
    ...
    "29072005poulomi"
      ...
      String sql = "SELECT * FROM public.books WHERE title ILIKE ? OR author ILIKE ?";//ILIKE used to do the case insensitive search
           PreparedStatement ps = con.prepareStatement(sql);//prepares the SQL query with placeholders (?) for parameters.
                 ps.setString(1, "%" + key + "%");
          ps.setString(2, "%" + key + "%");//This is used with the SQL LIKE operator for searching.
            ResultSet rs = ps.executeQuery();//This line is used in JDBC to execute a SELECT query and store the result.
